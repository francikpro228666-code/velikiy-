...
class Transaction {
    ...
    // ========== МЕТОДЫ ДЛЯ ОТЧЁТОВ (3.6) ==========
    
    /**
     * Получить суммы по категориям за период (доходы/расходы раздельно)
     * @param int $userId
     * @param string $startDate
     * @param string $endDate
     * @return array ['income' => [...], 'expense' => [...]]
     */
    public function getGroupedByCategory($userId, $startDate, $endDate) {
        $sql = "SELECT c.name, c.type, COALESCE(SUM(t.amount), 0) as total 
                FROM categories c
    ...
    /**
     * Получить сводку доходов/расходов за период
     * @param int $userId
     * @param string $startDate
     * @param string $endDate
     * @return array ['total_income' => ..., 'total_expense' => ..., 'balance' => ...]
     */
    public function getSummary($userId, $startDate, $endDate) {
        $data = $this->getGroupedByCategory($userId, $startDate, $endDate);
        $totalIncome = array_sum(array_column($data['income'], 'total'));
        $totalExpense = array_sum(array_column($data['expense'], 'total'));
        return [
